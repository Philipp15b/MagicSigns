...
public class SignEditListener implements Listener {
	...
	public void onBlockPlace(BlockPlaceEvent event) {
		...
		// signs.
		if (!isSign(event.getBlockPlaced().getType()) || !isSign(event.getBlockAgainst().getType()))
			return;
		...
		if (mode == EditMode.MASK_MAGIC_SIGNS
				&& signEdit.plugin.getSignManager().getSign(new BlockLocation(event.getBlockAgainst().getLocation())) == null)
			return;

		signEdit.registerEditSign(event.getBlockPlaced().getLocation(), event.getBlockAgainst().getLocation());
	}
	...
	public void onSignChange(SignChangeEvent event) {
		...
		Sign targetSign = (Sign) signEdit.getTargetBlock(event.getBlock()).getState();
		...
		boolean maskMagicSign = ((playerEditMode == EditMode.MASK_MAGIC_SIGNS || playerEditMode == EditMode.AUTO) && signEdit.plugin
				.getSignManager().getSign(new BlockLocation(targetSign.getLocation())) != null);
		...
		if (!maskMagicSign) {
			...
			if (playerEditMode == EditMode.MODIFY
					&& signEdit.plugin.getSignManager().getSign(new BlockLocation(targetSign.getLocation())) != null) {
				signEdit.plugin.getSignManager().removeSign(new BlockLocation(targetSign.getLocation()));
			}

			// call a new SignChangeEvent to inform other plugins
			signChange = new SignChangeEvent(targetSign.getLocation().getBlock(), event.getPlayer(), newLines);
	...
	public boolean willEditMagicSign(PlayerInteractEvent event) {
		if (signEdit.getEditMode(event.getPlayer()) == EditMode.NONE)
			return false;
		if (isSign(event.getClickedBlock().getType()) && event.getItem() != null && isSign(event.getItem().getType())) {
			return true;
		}
		return false;
	}
